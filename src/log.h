...
#define xzl_unlikely(x)     __builtin_expect((x),0)
#define xzl_bug_on(condition) do { if (xzl_unlikely(condition)) xzl_bug("bug:condition failed"); } while (0)
#define xzl_bug_on_msg(condition, msg) do { if (xzl_unlikely(condition)) xzl_bug(msg); } while (0)
